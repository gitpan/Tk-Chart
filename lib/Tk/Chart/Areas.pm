  ...
  for my $IndexLegend ( 1 .. $CompositeWidget->{RefChart}->{Legend}{NbrLegend} ) {
    ...
    $CompositeWidget->bind(
      ...
      sub {
        ...
        $CompositeWidget->itemconfigure( $CompositeWidget->{RefChart}->{TAGS}{DashLines}, -fill => 'black', );
      }
    );
  }
...
sub _Legend {
  ...
  $CompositeWidget->{RefChart}->{Legend}{LengthOneLegend}
    = +$CompositeWidget->{RefChart}->{Legend}{SpaceBeforeCube}    # space between legend
    + $CompositeWidget->{RefChart}->{Legend}{WCube}               # Cube (width)
    + $CompositeWidget->{RefChart}->{Legend}{SpaceAfterCube}      # space after cube
    + $CompositeWidget->{RefChart}->{Legend}{WidthText}           # length of legend text
    ;
